...
use diesel::*;
use diesel_mssql::mssql::{BindValue, Mssql};
use diesel_mssql::ColumnData;
...
impl ToSql<MyType, Mssql> for MyEnum {
    fn to_sql<'b>(&'b self, out: &mut Output<'b, '_, Mssql>) -> serialize::Result {
        match *self {
            MyEnum::Foo => out.set_value(BindValue::Text("foo")),
            MyEnum::Bar => out.set_value(BindValue::Text("bar")),
        }
        Ok(IsNull::No)
    }
}

impl FromSql<MyType, Mssql> for MyEnum {
    fn from_sql(bytes: ColumnData<'_>) -> deserialize::Result<Self> {
        if let ColumnData::String(Some(value)) = bytes {
            if value == "foo" {
                return Ok(MyEnum::Foo);
            }
            if value == "bar" {
                return Ok(MyEnum::Bar);
            }
        };
        diesel::deserialize::Result::Err("Whoopsie".into())
    }
}
...
impl ToSql<MyTypeInCustomSchema, Mssql> for MyEnumInCustomSchema {
    fn to_sql<'b>(&'b self, out: &mut Output<'b, '_, Mssql>) -> serialize::Result {
        match *self {
...
impl FromSql<MyTypeInCustomSchema, Mssql> for MyEnumInCustomSchema {
    fn from_sql(bytes: ColumnData<'_>) -> deserialize::Result<Self> {
...
fn custom_type_schema_inference() {
    ...
    let other_ty = <Mssql as HasSqlType<OtherTy>>::metadata(conn);
    let public_ty = <Mssql as HasSqlType<PublicTy>>::metadata(conn);
    let inferred_ty = <Mssql as HasSqlType<InferredTy>>::metadata(conn);
