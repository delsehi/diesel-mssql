...
fn filter_after_joining() {
    ...
    let connection = &mut connection_with_sean_and_tess_in_users_table();
    diesel::sql_query(
        "SET IDENTITY_INSERT posts ON; INSERT INTO posts (id, title, user_id) VALUES
                       (1, 'Hello', 1), (2, 'World', 2); SET IDENTITY_INSERT posts OFF;",
    )
    .execute(connection)
    .unwrap();
...
#[test]
#[ignore = "An expression of non-boolean type cannot be specified in a context where a condition is expected"]
