...
fn group_by_generates_group_by_sql() {
    ...
    let expected_sql = "SELECT [users].[name] FROM [users] \
...
fn group_by_mixed_aggregate_column_and_aggregate_function() {
    ...
    let source = users::table
        ...
        .filter(users::hair_color.is_null());
    let expected_sql = "SELECT max([users].[id]), [users].[name] FROM [users] \
                            WHERE ([users].[hair_color] IS NULL) \
                            GROUP BY [users].[name] \
                            -- binds: []"
        .to_string();
...
// TODO: Not supported in SQL Server?
#[test]
#[ignore = "Columns need to be in the select statement even when using a group by"]
...
fn check_group_by_multiple_columns_in_group_by_clause_single_select() {
    ...
    let expected_sql = "SELECT [users].[name] \
...
fn check_group_by_multiple_columns_in_group_by_clause_complex_select() {
    ...
    let expected_sql = "SELECT [users].[name], [users].[hair_color], max([users].[id]) \
                            FROM [users] WHERE ([users].[id] IS NULL) \
                            GROUP BY [users].[name], [users].[hair_color] \
                            -- binds: []"
        .to_string();
...
#[test]
#[ignore = "Columns need to be in the group by even when using primary key"] // Not supported in SQL Server
fn check_group_by_multiple_tables() {
    ...
    let expected_sql = "SELECT [users].[name], [posts].[title], count([comments].[id]) \
                            FROM ([users] \
                            INNER JOIN ([posts] \
...
fn check_filter_with_group_by_subselect() {
    ...
    let expected_sql = "SELECT [posts].[user_id], [posts].[title] \
         FROM [posts] \
         WHERE (\
         [posts].[id] IN (\
         SELECT min([posts].[id]) \
